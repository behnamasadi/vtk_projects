...
public:
  ...
  vtkUserData initializeVTK(vtkRenderWindow *renderWindow) override {
    ...
    // Remember: QML can delete our underlying QSGNode (which calls this method)
    // at any time. We have to re-synchronize our Qt properties with our VTK
    // properties at any time. To this end we've added a "force" parameter to
    // our Qt property setter which is set true here in initializeVtk but is
    // defaulted false whenever QML (or other C++ code) invokes it.
    //
    // To see QML randomly delete our QSGNode, split horizontally, then split
    // vertically and then unsplit the smallest top-most view and observe the
    // console output.
    //
    ...
    // clang-format on
    //
    // Notice that there are 2 (two) 'destructed' messages but you only unsplit
    // once!! QML deleted both "small" QSGNodes and then created a new QSGNode
    // to fill the empty column.
    setSource(_source, true);

    // Note:  It is okay to store some non-graphical VTK objects in the
    // QQuickVTKItem instead of the vtkUserData but ONLY if they are accessed
    // from the qml-render-thread. (i.e. only in the initializeVTK,
    // destroyingVTK or dispatch_async methods)
  ...
  bool event(QEvent *ev) override {
    ...
    case QEvent::MouseButtonPress: {
      ...
      _click.reset(new QMouseEvent(e->type(), e->localPos(), e->windowPos(),
...
int main(int argc, char *argv[]) {
  ...
  engine.load(QUrl(QStringLiteral("qrc:/qml/dynamic_split_view_main.qml")));
...
#include "dynamic_split_view.moc"
